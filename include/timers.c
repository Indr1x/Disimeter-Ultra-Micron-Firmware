...
void reset_TIM_prescallers_and_Compare(void)
{
  ...
  TIM_PrescalerConfig(TIM4, (uint32_t) (SystemCoreClock / 1000) - 1, TIM_PSCReloadMode_Immediate);      // Делитель (1 тик = 10мс)
...
void timer9_Config(void)        // генерация накачки
{
  ...
  //TIM_BaseConfig.TIM_Period = 300;      // ИЗМЕРЕННО ОСЦЫЛОМ, 560!  Общее количество тиков (скваженность) 75мкс (частота накачки 1с/140мкс=** кГц)
#else
...
void tim4_Config()              // Модуль-А - поиск
{
  ...
  TIM_BaseConfig.TIM_Prescaler = (uint16_t) (SystemCoreClock / 1000) - 1;       // Делитель (1 тик = 1мс)
  TIM_BaseConfig.TIM_ClockDivision = 0;
  TIM_BaseConfig.TIM_Period = 100;      // Общее количество тиков
  TIM_BaseConfig.TIM_CounterMode = TIM_CounterMode_Up;
